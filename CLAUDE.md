# CLAUDE.md — audio-glyph-inference
...
These extend the global `~/.claude/CLAUDE.md` rules. Where they differ, the project rules win for this project.
...
When you're about to add a new class or family, use `/scaffold` (see `.claude/commands/scaffold.md`). Do NOT hand-roll the file layout.
...
Every file under `backend/src/simulation/transforms/` implements this exact protocol. See `backend/src/simulation/transforms/transform_base.py` for the definition and the `transform-protocol` skill under `.claude/skills/` for the invariant checklist.
...
When Phase 4 begins:
- Scaffold `frontend/` per the global CLAUDE.md §5 frontend stack.
...
Scientific Python (`torch`, `scipy`, `numba`, `opencv`) ships wheels built against glibc. Alpine's musl libc either breaks the wheels or forces source compilation at image build time. Use `python:3.11-slim` (Debian-based) per global CLAUDE.md §9.

### Launcher contract

The root-level `run_audio_glyph_inference.sh` and `run_audio_glyph_inference.bat` implement the global CLAUDE.md §4 launcher contract:
...
If the user reports the launcher looks different from `llm-knowledge-base`'s, check whether global CLAUDE.md has been updated since this project was scaffolded — the canonical template lives there.
...
### Rules (from global CLAUDE.md §7 plus project additions)
...
**The user manages all git state themselves.** Carried from global CLAUDE.md §10, reproduced here because it's project-load-bearing:
...
- The `.claude/settings.json` Stop hook emits a reminder if uncommitted changes exist at turn end. That reminder is for the user, not for you.
