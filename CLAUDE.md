...
**Agent harness configs:** the tracked agent wiring is shared — `.codex/` (hooks + slash commands) and `.agents/skills/`. `CLAUDE.md` (this file) and `AGENTS.md` are kept in sync as the Claude Code and Codex entrypoints respectively; a local `.claude/` mirror, if present, is gitignored.
...
Project application of global `~/.claude/CLAUDE.md` §19 `<security>`. The global section defines the fleet rule; this section names what it means in this repo.
