...
- CI fix: `backend/Dockerfile` dependency-install layer no longer uses bash process substitution (`<(...)`), which `/bin/sh` on `python:3.11-slim` rejects. Swapped to `uv pip compile -o /tmp/requirements.txt` + `uv pip install -r /tmp/requirements.txt`.
